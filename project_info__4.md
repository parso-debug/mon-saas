# ArtisanWeb — Skill “artisanweb-frontend” (Spécification pour skill-creator)

## Résumé
Cette “skill” est un assistant pour transformer des écrans/maquettes en composants **React 18 (CRACO)** conformes à l’UI kit **shadcn/ui** et à **Tailwind** (**mobile-first**, **pas de CSS custom**), tout en respectant l’architecture du frontend ArtisanWeb. Elle guide le diagnostic UX/UI d’une page existante, la génération de nouvelles pages SaaS, le refactor vers shadcn/ui et l’intégration propre des appels API côté frontend avec gestion **loading/error** et **toasts**. Elle ne touche jamais au backend ni à la logique auth/routes API.

## Architecture (niveau “skill”)
- **Pattern** : pipeline de travail guidé (analyse → plan → code → responsive/accessibilité → test) avec contraintes strictes de style/tech.
- **Sub-systèmes** :
  - Diagnostic & matching de composants (shadcn/ui existants)
  - Génération/refactor React + Tailwind (mobile-first)
  - Intégration API côté frontend (JWT via mécanismes existants + états UI)
  - Contrôle qualité (FR-only côté UI, accessibilité, paliers responsive)

## Emplacement (à créer en Act Mode)
Dossier requis :
- **Windows** : `C:\Users\admin\.claude\skills\artisanweb-frontend\`
- **Shorthand** : `~/.claude/skills/artisanweb-frontend/`

Fichier requis :
- `SKILL.md` (obligatoire)

## Contenu proposé pour `SKILL.md` (copier tel quel)
...
- Préserver l’architecture CRACO (ne pas proposer Next/Vite migration).
...
## Todo (cohérent avec la demande “skill-creator”)
- [x] Définir le nom de la skill (`artisanweb-frontend`)
- [x] Définir objectif, périmètre, entrées et sorties
- [x] Définir contraintes absolues (FR-only UI, shadcn/ui, Tailwind, mobile-first)
- [x] Rédiger `SKILL.md` prêt à être copié
- [ ] (Act Mode) Créer `~/.claude/skills/artisanweb-frontend/`
- [ ] (Act Mode) Écrire `SKILL.md` dans le dossier
